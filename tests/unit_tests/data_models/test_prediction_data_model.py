...
from src.data_models.prediction_data_model import (
...
def test_validate_data_with_missing_class_columns(schema_provider, predictions):
    ...
    with pytest.raises(ValueError) as exc_info:
        _ = validate_predictions(
            predictions_missing_class,
            schema_provider
        )
        exit_mock.assert_called_once_with(1)
    assert "ValueError: Malformed predictions file. Target field(s) " \
        in str(exc_info.value)
    assert "missing in predictions file." in str(exc_info.value)


def test_validate_data_with_invalid_probabilities(
        schema_provider, predictions):
    ...
    with pytest.raises(ValueError) as exc_info:
        _ = validate_predictions(
            predictions_invalid_probabilities,
            schema_provider
        )
    assert "ValueError: Invalid probabilities in predictions file. " \
        in str(exc_info.value)
    assert "All probabilities should be between 0 and 1, inclusive." \
        in str(exc_info.value)
